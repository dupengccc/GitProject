...
namespace Service.ServiceImp.SysManage
{
    /// <summary>
        ...
        /// </summary>
        public class DepartmentManage : RepositoryBase<Domain.SYS_DEPARTMENT>, IDepartmentManage
        {
            /// <summary>
            /// 自动创建部门编号
            /// add yuangang by 2016-05-19
            ...
            public string FormatCode(int dptCode)
            {
               ...
               // string _strCode = string.Empty;
                ...
                //return string.Empty;
                ///简单业务逻辑
                int codeLen = dptCode.ToString().Length;
                if (codeLen >= 3) return dptCode.ToString();
                string result = dptCode.ToString().PadLeft(3, '0');
                return result;
            }
                catch (Exception ex)
                {
